	...
	class OceanWP_WooCommerce_Config {
		...
		public static function loop_product_thumbnail() {
			if ( function_exists( 'wc_get_template' ) ) {
				...
				// Get entry product media template parts
					...
					if ( '' !== $data[ $key ] && in_array( 'state', $format ) ) {
						...
						if ( ! empty( $valid_states ) && is_array( $valid_states ) && sizeof( $valid_states ) > 0 ) {
							...
							if ( isset( $valid_state_values[ $data[ $key ] ] ) ) {
								// With this parts we consider state value to be valid as well, convert it to the state key for the valid_states check below.
